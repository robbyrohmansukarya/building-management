...
class Perbaikan extends Login_Controller
{
    ...
    public function index(){
        ...
        $request = $this->Perbaikan_model->er_get_all($ideksekutor)->result();

		$data = array(
            'request_data' => $request,
            'view' =>'eksekutor/perbaikan/request_list'
        );
    ...
    public function read($id_request){
        ...
        if($queryReq){
            $data = array(
			'action' 			=> site_url('eksekutor/perbaikan/progres'),
			...
			'id_klasifikasi' 	=> $queryReq->id_klasifikasi,
			'progress'		 	=> $this->Perbaikan_model->get_progres($queryReq->ideksekutor, $id_request)->result(),
			'view'				=>'eksekutor/perbaikan/request_read',
	 		);
		$this->load->view('template', $data);
        }else{
	...
	function progres(){
		...
		if($nama_photo_lokasi != ''){
			...
			$nmphoto_lokasi		= 'l'.time().'.'.$extension_l;
				...
				redirect(site_url('eksekutor/perbaikan/read/'.$id_request.'/extension'));
			}
		}
		...
		$this->Perbaikan_model->update_status($id_request, $data2);
		$this->Perbaikan_model->insert_progres($data);
		...
		*/
		...
		redirect(site_url('eksekutor/perbaikan/read/'.$id_request.'/success'));
	}

   public function photo($id_request){
		$q 		= $this->Perbaikan_model->get_by_id($id_request);
		if($q){
			$data = array(
				'view' 			=> 'eksekutor/perbaikan/link_pic',
    ...
    public function delete_progres($id_progres, $id_request){
        $row = $this->Perbaikan_model->progres_by_id($id_progres);
        ...
        if($row){
            ...
            $this->Perbaikan_model->delete_progres($id_progres);
			redirect(site_url('eksekutor/perbaikan/read/'.$id_request.'/delete'));
        }else{
            //$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('eksekutor/perbaikan/read/'.$id_request.'/detail-request'));
        }
    }
...
/* Location: ./application/controllers/eksekutor/perbaikan.php */
